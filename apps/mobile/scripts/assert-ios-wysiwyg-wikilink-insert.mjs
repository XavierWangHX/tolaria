...
function printHelp() {
  console.log(`Assert native iOS Simulator WYSIWYG wikilink, person mention, emoji, and slash-command insertion.
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS WYSIWYG wikilink, person mention, emoji, and slash-command insertion')
...
async function main() {
  ...
  console.log('Native iOS WYSIWYG wikilink, person mention, emoji, and slash-command insertion proof passed.')
}
