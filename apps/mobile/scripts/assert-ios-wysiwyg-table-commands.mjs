...
import {
  ...
  nativeWysiwygTableCommandMutationLogPrefix,
  parseNativeWysiwygTableCommandMutationProofs,
} from '../src/qa/nativeWysiwygTableCommandMutationProbe.ts'
...
function printHelp() {
  console.log(`Assert native iOS Simulator WYSIWYG table command mutations.

Usage:
  node apps/mobile/scripts/assert-ios-wysiwyg-table-commands.mjs [options]
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS WYSIWYG table command mutations')
  terminateExpoGo(device)
  await sleep(500)
  run('xcrun', ['simctl', 'openurl', device, tableCommandMutationProbeUrl(openUrl)])
  await sleep(Math.max(waitMs, minimumOpenWaitMs))
}

function tableCommandMutationProbeUrl(openUrl) {
  return appendQueryParam(
    appendQueryParam(openUrl, 'wysiwygTableCommandMutationProbe', '1'),
    'qaRun',
    Date.now().toString(),
  )
}
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeWysiwygTableCommandMutationLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  const { failures } = await waitForNativeProof({
    assertProofs: assertNativeWysiwygTableCommandMutationProofs,
    collectLogs: () => collectSimulatorLogs(device, {
      last: config.last,
      start: logStart,
    }),
    parseProofs: parseNativeWysiwygTableCommandMutationProofs,
    timeoutMs: config.openUrl ? proofPollTimeoutMs : 0,
  })
  if (failures.length > 0) {
    throw new Error(
      `Native WYSIWYG table command mutation proof failed:\n${formatNativeWysiwygTableCommandMutationFailures(failures)}`,
    )
  }

  console.log('Native iOS WYSIWYG table command mutation proof passed.')
}
