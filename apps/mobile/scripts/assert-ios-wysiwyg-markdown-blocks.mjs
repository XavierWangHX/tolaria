...
import {
  ...
  nativeWysiwygMarkdownBlockLogPrefix,
  parseNativeWysiwygMarkdownBlockProofs,
} from '../src/qa/nativeWysiwygMarkdownBlockProbe.ts'

const defaultExpoGoBundleId = 'host.exp.Exponent'
const defaultLogWindow = '90s'
const proofPollTimeoutMs = 14000

function printHelp() {
  console.log(`Assert native iOS Simulator WYSIWYG markdown block insertion.

Usage:
  node apps/mobile/scripts/assert-ios-wysiwyg-markdown-blocks.mjs [options]
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS WYSIWYG markdown block insertion')
  terminateExpoGo(device)
  await sleep(500)
  run('xcrun', ['simctl', 'openurl', device, markdownBlockProbeUrl(openUrl)])
  await sleep(Math.max(waitMs, 10000))
}

function markdownBlockProbeUrl(openUrl) {
  return appendQueryParam(appendQueryParam(openUrl, 'wysiwygMarkdownBlockProbe', '1'), 'qaRun', Date.now().toString())
}
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeWysiwygMarkdownBlockLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  const { failures } = await waitForNativeProof({
    assertProofs: assertNativeWysiwygMarkdownBlockProofs,
    collectLogs: () => collectSimulatorLogs(device, {
      last: config.last,
      start: logStart,
    }),
    parseProofs: parseNativeWysiwygMarkdownBlockProofs,
    timeoutMs: config.openUrl ? proofPollTimeoutMs : 0,
  })
  if (failures.length > 0) {
    throw new Error(`Native WYSIWYG markdown block insertion proof failed:\n${formatNativeWysiwygMarkdownBlockFailures(failures)}`)
  }

  console.log('Native iOS WYSIWYG markdown block insertion proof passed.')
}
