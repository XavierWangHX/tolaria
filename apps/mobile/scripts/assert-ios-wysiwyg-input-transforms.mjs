...
import {
  ...
  nativeWysiwygInputTransformLogPrefix,
  parseNativeWysiwygInputTransformProofs,
} from '../src/qa/nativeWysiwygInputTransformProbe.ts'
...
function printHelp() {
  console.log(`Assert native iOS Simulator WYSIWYG input transforms.

Usage:
  node apps/mobile/scripts/assert-ios-wysiwyg-input-transforms.mjs [options]
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS WYSIWYG input transforms')
  terminateExpoGo(device)
  await sleep(500)
  run('xcrun', ['simctl', 'openurl', device, inputTransformProbeUrl(openUrl)])
  await sleep(Math.max(waitMs, minimumOpenWaitMs))
}

function inputTransformProbeUrl(openUrl) {
  return appendQueryParam(appendQueryParam(openUrl, 'wysiwygInputTransformProbe', '1'), 'qaRun', Date.now().toString())
}
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeWysiwygInputTransformLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  const { failures } = await waitForNativeProof({
    assertProofs: assertNativeWysiwygInputTransformProofs,
    collectLogs: () => collectSimulatorLogs(device, {
      last: config.last,
      start: logStart,
    }),
    parseProofs: parseNativeWysiwygInputTransformProofs,
    timeoutMs: config.openUrl ? proofPollTimeoutMs : 0,
  })
  if (failures.length > 0) {
    throw new Error(`Native WYSIWYG input transform proof failed:\n${formatNativeWysiwygInputTransformFailures(failures)}`)
  }

  console.log('Native iOS WYSIWYG input transform proof passed.')
}
