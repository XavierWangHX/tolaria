...
import {
  ...
  nativeTableOfContentsLogPrefix,
  parseNativeTableOfContentsProofs,
} from '../src/qa/nativeTableOfContentsProbe.ts'
...
function printHelp() {
  console.log(`Assert native iOS Simulator table-of-contents scrolling.

Usage:
  node apps/mobile/scripts/assert-ios-table-of-contents.mjs [options]
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS table of contents')
  terminateExpoGo(device)
  await sleep(500)
  run('xcrun', ['simctl', 'openurl', device, tableOfContentsProbeUrl(openUrl)])
  await sleep(Math.max(waitMs, minimumOpenWaitMs))
}

function tableOfContentsProbeUrl(openUrl) {
  return appendQueryParam(
    appendQueryParam(
      appendQueryParam(openUrl, 'tableOfContentsProbe', '1'),
      'layoutProbe',
      '1',
    ),
    'qaRun',
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeTableOfContentsLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  const { failures } = await waitForNativeProof({
    assertProofs: assertNativeTableOfContentsProofs,
    collectLogs: () => collectSimulatorLogs(device, {
      last: config.last,
      start: logStart,
    }),
    parseProofs: parseNativeTableOfContentsProofs,
    timeoutMs: config.openUrl ? proofPollTimeoutMs : 0,
  })
  if (failures.length > 0) {
    throw new Error(`Native table of contents proof failed:\n${formatNativeTableOfContentsFailures(failures)}`)
  }

  console.log('Native iOS table of contents proof passed.')
}
